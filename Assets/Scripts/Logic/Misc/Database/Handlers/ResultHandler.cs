...
public class ResultHandler : MonoBehaviour
{
    ...
    public List<Result> GetResultsByBoardSize(int size)
    {
        ...
        using (var connection = new SqliteConnection(database.databaseName))
        {
            ...
            using (var command = connection.CreateCommand())
            {
                command.CommandText = "SELECT * FROM `Result` INNER JOIN `User_Result` ON User_Result.result_id = Result.id WHERE Result.board_size = @size AND User_Result.user_id = @user_id;";
                command.Parameters.AddWithValue("@user_id", database.loggedInUser.id);
    ...
    public List<UserResultDTO> GetResultsByBoardSizeAndDifficulty(int difficulty,int size)
    {
        ...
        using (var connection = new SqliteConnection(database.databaseName))
        {
            ...
            using (var command = connection.CreateCommand())
            {
                command.CommandText = "SELECT User.username as username, Result.moved as moved, Result.time as time FROM `Result` LEFT JOIN `User_Result` ON User_Result.result_id = Result.id LEFT JOIN `User` ON User.id = User_Result.user_id WHERE board_size = @size AND difficulty = @difficulty ORDER BY moved DESC,time DESC;";
                command.Parameters.AddWithValue("@size", size);
